...
def generate_mesh(
    x: np.ndarray, eta: np.ndarray, L: float, h0: float, mesh_size: float
):
    ...
    lines += [
        gmsh.model.geo.addLine(pts[-1], pL),
...
def plot_domain_and_mesh(
    msh_file: str, x_full: np.ndarray, eta_full: np.ndarray, h0: float
):
...
def parse_args():
    ...
    parser.add_argument(
        "-i", "--input", default="data/data.csv", help="Path to input CSV file"
    )
    parser.add_argument(
        "-m",
        "--msh",
        default="mesh/mesh.msh",
        help="Output Gmsh .msh file (default: mesh/mesh.msh)",
    )
    parser.add_argument(
        "-x",
        "--xdmf",
        default="mesh/mesh.xdmf",
...
def print_mesh_statistics(msh_file: str):
    ...
    edge_lengths = np.linalg.norm(
        points[edge_list[:, 0]] - points[edge_list[:, 1]], axis=1
    )
    ...
    print(
        f"  Aspect ratio (R/r): min {aspect_ratios.min():.4g}, max {aspect_ratios.max():.4g}, mean {aspect_ratios.mean():.4g}, std {aspect_ratios.std():.4g}"
    )
    print(
        f"  Min angle: min {min_angles.min():.4g}°, max {min_angles.max():.4g}°, mean {min_angles.mean():.4g}°, std {min_angles.std():.4g}°"
    )
    print(
        f"  Max angle: min {max_angles.min():.4g}°, max {max_angles.max():.4g}°, mean {max_angles.mean():.4g}°, std {max_angles.std():.4g}°"
    )
    print("=================================\n")
